...
if (!nativeBinding || forceWasi) {
  ...
  if (!wasiBindingLoaded && (!__napiWasiFlavorRequested || __napiWasiFlavor === "wasm32-wasi")) {
    ...
    try {
      candidateError = __napiWasiResolveCandidate('./awrit-native-rs.wasi.cjs', false, ["./awrit-native-rs.wasm32-wasi.debug.wasm","./awrit-native-rs.wasm32-wasi.wasm"])
  ...
  if (!wasiBindingLoaded && (!__napiWasiFlavorRequested || __napiWasiFlavor === "wasm32-wasi")) {
    ...
    let candidateFailed = false
    try {
      candidateError = __napiWasiResolveCandidate('awrit-native-rs-wasm32-wasi', true, undefined)
      candidateFailed = candidateError !== null
      if (!candidateFailed) {
        if (process.env.NAPI_RS_ENFORCE_VERSION_CHECK && process.env.NAPI_RS_ENFORCE_VERSION_CHECK !== '0') {
          const bindingPackageVersion = require('awrit-native-rs-wasm32-wasi/package.json').version
          if (bindingPackageVersion !== '2.0.3') {
            throw new Error(`WASI binding package version mismatch, expected 2.0.3 but got ${bindingPackageVersion}. You can reinstall dependencies to fix this issue.`)
          }
        }
